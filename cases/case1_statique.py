...
def run_discrete_brute_force(params, progress_callback=None):
    ...
    for i, config in enumerate(combinations):
        ...
        if progress_callback:
            progress_callback(i, total_steps, f"Test {r_cr} anneau(x) à {int(d_cr*100)}%")
        ...
        if np.sum(tags == "CR") == 0:
            # Si la combinaison ne génère aucune barre (cœur trop petit), on zappe
            for fuel in fuels:
        ...
        for fuel in fuels:
            ...
            K = assemble_stiffness(nn, len(conn), 3, len(w), conn, det, w, jacs, gradN, c_D)

            z_eq = pos_rodBar_init(mesh, conn, det, w, N, get_material_properties, K, nn, user_map, nodes_bc)
